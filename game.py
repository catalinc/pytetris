...
from board import SHAPE_I, SHAPE_O, ALL_SHAPES, DIRECTION_DOWN, DIRECTION_LEFT, DIRECTION_RIGHT, Piece, Board
...
class State(object):
    ...
    def next_piece(self):
        ...
        row = col = 0
        if shape in (SHAPE_I, SHAPE_O):
            col = self.board.cols / 2
        self.piece = Piece(shape, row, col)

    def next_shape(self):
        if not self.shapes:
            self.shapes = list(ALL_SHAPES)
    ...
    def drop_piece(self):
        while not self.board.has_landed(self.piece):
            self.move_piece(DIRECTION_DOWN)
...
class Game(object):
    ...
    def draw_next_piece(self, y):
        ...
        piece = Piece(self.state.next_shape(), row, col)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_DOWN:
                    self.state.move_piece(DIRECTION_DOWN)
                if event.key == pygame.K_LEFT:
                    self.state.move_piece(DIRECTION_LEFT)
                if event.key == pygame.K_RIGHT:
                    self.state.move_piece(DIRECTION_RIGHT)
